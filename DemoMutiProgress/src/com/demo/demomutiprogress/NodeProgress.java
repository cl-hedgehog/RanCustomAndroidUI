...
//	private int textSize;  //字体大小
	...
	public NodeProgress(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		TypedArray mTypedArray = context.obtainStyledAttributes(attrs,R.styleable.NodeProgress);
		nodesNum = mTypedArray.getInteger(R.styleable.NodeProgress_nodesNum, 1); //默认一个节点
		nodeRadius = mTypedArray.getDimensionPixelSize(R.styleable.NodeProgress_nodeRadius, 10); //节点半径
		progressingDrawable = mTypedArray.getDrawable(R.styleable.NodeProgress_progressingDrawable);
		unprogressingDrawable = mTypedArray.getDrawable(R.styleable.NodeProgress_unprogressingDrawable);
		progresFailDrawable = mTypedArray.getDrawable(R.styleable.NodeProgress_progresFailDrawable);
		progresSuccDrawable = mTypedArray.getDrawable(R.styleable.NodeProgress_progresSuccDrawable);
		processingLineColor = mTypedArray.getColor(R.styleable.NodeProgress_processingLineColorBg, DEFAULT_LINE_COLOR);
		currNodeState = mTypedArray.getInt(R.styleable.NodeProgress_currNodeState, 1);
		currNodeNO = mTypedArray.getInt(R.styleable.NodeProgress_currNodeNo, 1);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		nodes = new ArrayList<NodeProgress.Node>();
